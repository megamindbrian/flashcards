...
if (process.argv.length >= 5) {
    ...
    if (_.isObject(value)) {
      ...
      return () => {
        ...
        return agent('GET', apiUrl({
          ...
          var decoded = entities.decodeXML(text);
          if (languageKey === 'en') {
            decoded = decoded.replace(/./ig, '#');
          }
          _.set(accumulator, keyChain, decoded);
  ...
  Promise.all(_.reduce(destinationCodes, (sum, languageKey) => {
    const fileName = _.template(os.tmpdir() + '/<%= languageKey %>-<%= timeStamp %>.json')({
      languageKey,
      timeStamp: moment().unix()
    });

    //Starts with the top level strings
    return sum.concat(_.reduce(iterLeaves(
      JSON.parse(fs.readFileSync(path.resolve(inputFile), 'utf-8')),
      undefined,
      undefined,
      languageKey), (promiseChain, fn) => {
